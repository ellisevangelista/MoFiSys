...
<html> 
    ...
    <body>
        ...
        <div class ="welcome">
            ...
            Welcome, <%out.print(rs.getString("FNAME").trim());String cmt = rs.getString("CMT");sesh.setAttribute("cmt", cmt);%>!</p>
            ...
            <div class="dropdown" style="position:absolute;right:6px;top:5px;">
                ...
                <div class="dropdown-content">
                    <a href="javascript:void();" onclick="myAcc();">Account Settings</a>
   ...
   <script>
    ...
    <script>
            ...
            <table>
                <tr>
                    ...
                    <td>
                        ...
                        &nbsp;&nbsp;
                        <input type="radio" name="doc_type"  value="Outgoing"  id="outCheckRep"><text class ="b-text">&nbsp;Outgoing</text>
        ...
        <div id ="viewBooks" style="display: none">
            ...
            <iframe src="viewBooks?pg=1" align="top" width="650" height="1000" scrolling="no">Please update your browser.</iframe>
            </div>
        </div>
        ...
        <div id ="myacc" style="display: none">
                    <div class="bodybox">
                        <div class="insrt-mainhdr" style="float:right;">
                        <text class="title">MY ACCOUNT</text>
                    </div>
                        <br><br>   
           <form action="updateNonAdmin" name="myaccount" method="POST" id="myform" target="myaccframe">
               <iframe style="display: none;" id="iframe" name="myaccframe"></iframe>
               <p class="b-text">Committee: <strong><%=rs.getString("CMT").trim()%></strong><br><br>
                First Name: <strong><%=rs.getString("FNAME").trim()%></strong><br/><br/>
                Last Name: <strong><%=rs.getString("LNAME").trim()%></strong><br /><br/>
                Username: <strong><%=rs.getString("USERNAME").trim()%></strong><br /><br/></p>
               <text class= "bl-text">Password:<br></text> 
               <input type="password" name="pass" id="pass" value="<%=rs.getString("PASS").trim()%>"title="Password must contain at least 8 characters, including UPPER/lowercase and numbers" type="password" required pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" name="pwd1" 
                onchange="this.setCustomValidity(this.validity.patternMismatch ? this.title : '');"/>
               <br><br>
               <text class= "bl-text">Confirm Password:<br></text> 
               <input type="password" name="cpass" id="cpass" title="Password must contain at least 8 characters, including UPPER/lowercase and numbers" type="password" required pattern="(?=.*\d)(?=.*[a-z])(?=.*[A-Z]).{8,}" name="pwd1" 
                onchange="this.setCustomValidity(this.validity.patternMismatch ? this.title : '');"/>
                    <input type="hidden" name="user" value="<%=rs.getString("USERNAME").trim()%>"/><input type="hidden" name="phase" value="one"/>
                    <button type="submit" class="lilbutton" onclick="validatePassForm()"/>Change Password</button><br/>
           <script>
               ...
               function validatePassForm()
                {
                var chpw=document.forms["myaccount"]["pass"].value;
                var chpw2=document.forms["myaccount"]["cpass"].value;
                if (chpw==null || chpw=="", chpw2==null || chpw=="", password.value != confirm_password.value){
                swal("Oops...", "Something went wrong! Please try again and make sure to fill out the important fields before submitting.", "error");;
                }     
                else if (chpw!=null || chpw!="", chpw2!=null || chpw2!="") {
                swal("Success!", "Password successfully changed! Please log in again to verify.", "success");;
                window.setTimeout(location.reload.bind(location), 4000);
                };              
                }
           </script>
